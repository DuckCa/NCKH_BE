...
const getItemLists = async () => {
  const ItemCatList = await Item.find();
...
const defaultDataCountItem = async () => {
  ...
  try {
      ...
      if (Item.category && Item.category.length > 0) {
        ...
        Item.category.forEach((catId) => {
          categoryCountMap.set(
            catId.toString(),
            (categoryCountMap.get(catId.toString()) || 0) + 1
          );
        });
      }
    });
    ...
    for (let Categories of categoryList) {
      const count = categoryCountMap.get(Categories._id.toString()) || 0;

      console.log(
        `🔄 Cập nhật CategoryId: ${Categories._id} với TotalItem: ${count}`
      );
